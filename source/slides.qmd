...
**Companion Website:** [Link in chat] — or scan now:

![](images/website.png){width=140 fig-align="center"}

::: {.notes}
The core theme today has two halves: think WITH AI, not THROUGH AI — and the conversation IS the thinking. But the destination is the human edge: when everyone has AI, the differentiator is what only you bring. Reverse Prompting and more prompts are on the companion site. Give the room 30 seconds to scan the QR — every prompt from today is on the top of that site, so nobody needs to copy code off slides.
:::
...
::: {.notes}
Switch to browser and run this in Gemini. Watch it produce slick, forgettable marketing-speak — "unforgettable evening of food, fun and festivities". Could be any event, anywhere. This is the average — and the average is free.
:::
...
::: {.notes}
Run this in the same chat. Same model, same event — now it sounds like a person: the "tutorial groups too nervous to mingle" insight, the dumpling guy. The AI did not get smarter. You gave it your edge — context and voice. The difference IS the edge. For assignments: if your marker could swap your essay with any other AI draft, what did you submit? Paste 100 words of your own writing and have AI critique it, not write it. Style Mirror (companion site) makes this permanent.
:::
...
## The Trust Tool — Where Does a Human Stay in the Loop? {.center}

Two questions before you act on any AI output:
...
| **Must be precise**   | Verify before you ship            | **A human owns it. AI is a first draft.** |

**Practice:** the Trust Tool game — on the companion site.

::: {.notes}
Deloitte, in one line: a red-quadrant output treated as green. The model didn't fail uniquely — the placement failed. Almost every public AI failure is someone treating a must-be-precise, high-stakes output as if it were an average, low-stakes one. The trust tool makes placement a conscious act: average or precise, small or large? Green corner: lean in, let AI run. Red corner: a human owns the decision. For assignments: your references and figures are red-quadrant — verify those before anything else. The interactive game on the companion site serves up scenarios to place on the grid.
:::
